...
이슈 상황,원인 분석,해결 방법
Permission Denied,관리자 권한 없이 시스템 파일 수정 시도,sudo 명령어를 사용하여 권한 부여 후 실행

Window Resize Error,iptraf-ng 실행 시 터미널 창 크기가 너무 작음,"터미널 창을 가로 80, 세로 24 이상으로 확대 후 실행"

Address already in use,8000번 포트를 이전 프로세스(runserver)가 점유 중,sudo fuser -k 8000/tcp로 기존 프로세스 강제 종료

Unit cannot be reloaded,Nginx가 중지(dead)된 상태에서 reload 시도,sudo systemctl start nginx로 서비스 먼저 기동

Old Page Displayed,Nginx 캐시 또는 이전 실습(Apache) 프로세스 간섭,
systemctl stop httpd 및 브라우저 강력 새로고침(Ctrl+Shift+R)

Hardware Noise (Fan),부하 테스트 중 CPU 온도 급상승으로 팬 소음 발생,sudo pkill -9 hping3로 부하 프로세스 즉시 종료
